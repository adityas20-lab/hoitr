...
def triplet_nms(hoi_list, iou_threshold=0.5):
    hoi_list.sort(key=lambda x: x['h_cls'] * x['o_cls'] * x['i_cls'], reverse=True)
    mask = [True] * len(hoi_list)
    for idx_x in range(len(hoi_list)):
        if not mask[idx_x]:
        ...
        for idx_y in range(idx_x + 1, len(hoi_list)):
            if not mask[idx_y]:
                continue
            if (IoU(hoi_list[idx_x]['h_box'], hoi_list[idx_y]['h_box']) > iou_threshold and
                IoU(hoi_list[idx_x]['o_box'], hoi_list[idx_y]['o_box']) > iou_threshold and
                hoi_list[idx_x]['i_name'] == hoi_list[idx_y]['i_name'] and
                hoi_list[idx_x]['o_name'] == hoi_list[idx_y]['o_name']):
                mask[idx_y] = False
    return [hoi for idx, hoi in enumerate(hoi_list) if mask[idx]]
...
def parse_object_box(org_cid, org_box, org_cls, img_size, coco_instance_id_to_name):
    ...
    n_cls = org_cls
    n_name = coco_instance_ID_to_name_hico[int(cid)]
...
def main():
    ...
    if args.img_sheet is None:
        img_path_list = [
            'D:/s20/HoiTransformer/data/hico/images/test/test_008796.jpg',
            'D:/s20/HoiTransformer/data/hico/images/test/test_008797.jpg'
        ]
    else:
